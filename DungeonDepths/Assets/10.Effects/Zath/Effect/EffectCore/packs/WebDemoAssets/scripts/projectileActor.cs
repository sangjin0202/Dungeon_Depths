...
public class projectileActor : MonoBehaviour {
   ...
   // public ParticleSystem muzzleflare;
    ...
    public void Fire()
    {
		...
		float _p = Random.Range(bombList[bombType].min, bombList[bombType].max) * 0.1f;
		rocketInstance.AddForce(spawnLocator.forward * _p);
		...
		if (bombList[bombType].shotgunBehavior)
        {
            for(int i = 0; i < bombList[bombType].shotgunPellets ;i++ )
            {
				...
				// Quaternion.Euler(0,90,0)
				//float _p = Random.Range(bombList[bombType].min, bombList[bombType].max) * 0.01f;
				//rocketInstanceShotgun.AddForce(shotgunLocator[i].forward * _p);
				rocketInstanceShotgun.AddForce(shotgunLocator[i].forward * _p);
			}
        }
